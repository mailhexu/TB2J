...
class BandsPlot:
    ...
    def plot(self, filename=None):
        ...
        for band in self.bands.T:
            axs.plot(kdata, band, linewidth=self.linewidth, color=self.color, **self.plot_options)

        bmin, bmax = self.bands.min(), self.bands.max()
        ymin, ymax = bmin - 0.05*np.abs(bmin-bmax), bmax + 0.05*np.abs(bmax-bmin)
        ...
        axs.vlines(x=kpoint_labels[0], ymin=ymin, ymax=ymax, color='black', linewidth=self.linewidth/5)
