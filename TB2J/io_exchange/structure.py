...
class BaseMagneticStructure:
    ...
    def __init__(
        ...
        if atoms is not None:
            if any(arg is not None for arg in [cell, elements, positions]):
                warn("WARNING: 'atoms' overrides the 'cell', 'elements', and 'positions' arguments.")
    ...
    def positions(self, value):
        ...
        if posarray.shape != (natoms, 3):
            raise ValueError("'positions' must have a (natoms, 3) shape, where natoms is the length of 'elements'. Make sure to set 'elements' first.")
    ...
    def magmoms(self, value):
        ...
        if self.collinear and magarray.shape != (len(self._positions), ):
            raise ValueError("'magmoms' must be a 1dim array with the same length as 'positions'. Make sure to set 'positions' first.")
        elif not self.collinear and magarray.shape != self._positions.shape:
            raise ValueError("'magmoms' must have the same shape as 'positions'. Make sure to set 'positions' first.")
    ...
    def propagation_vector(self, value):
        ...
        if Q.shape != (3,):
            raise ValueError("The 'propagation_vector' must be a vector with 3 numbers.")
