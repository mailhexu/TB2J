    ...
    def _prepare_distance(self):
        ...
        for ispin, iatom in enumerate(ind_matoms):
            for jspin, jatom in enumerate(ind_matoms):
                # Skip if ispin0_only is True and neither ispin nor jspin is 0
                if self.ispin0_only and ispin != 0 and jspin != 0:
                    continue
                if iatom in self.list_iatom and jatom in self.list_jatom:
                    for R in self.Rlist:
                        ...
                        if self.Rcut is None or distance < self.Rcut:
                            self.distance_dict[(tuple(R), ispin, jspin)] = (
                                vec,
                                distance,
                            )
