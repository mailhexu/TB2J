...
* :math:`J^{iso} \mathbf{I}` is the isotropic exchange (diagonal matrix)
* :math:`\mathbf{J}^{DMI}` is the antisymmetric DMI tensor:

  .. math::

      \mathbf{J}^{DMI} = \begin{pmatrix}
      0 & D_z & -D_y \\
      -D_z & 0 & D_x \\
      D_y & -D_x & 0
      \end{pmatrix}

* :math:`\mathbf{J}^{ani}` is the symmetric anisotropic exchange tensor
